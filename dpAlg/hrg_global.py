...
class HRGbuilder:
    ...
    def learnDend(
        ...
        for epoch in range(epochs):
            #         if stopCondition(ldList, epoch):
            #             break
            for i in range(N):
                # sample a new internal node name from the list of nodeNames
                randNodeName = np.random.choice(nodeNames[1:], 1)[0]
                # retrieve the node from the map using its name
                randNode = nodeMap[randNodeName]
                # take its parent
                randNodeParent = randNode.parent
                # config tells you which of the two local swaps to make
                ...
                if config == 0:
                    ...
                    exponent = lognum_u + lognum_d - logdenom_u - logdenom_d
                    if self._dp:
                        exponent = (lognum_u + lognum_d - logdenom_u - logdenom_d) * (
                            self._eps_l / (2 * math.log(0.75 * (N ** 2)))
                        )
                    if exponent < -30:
                        exp_ld_ratio = 0.0
                    elif exponent > 30:
                        ...
                        exp_ld_ratio = math.exp(exponent)
            ...
            if epoch % (max(1, epochs / 100)) == 0:
                print(
                    "Epoch: {:5d}, logL: {} time: {:6.4f}".format(epoch, LD_old, time.time() - startTime)
                )
